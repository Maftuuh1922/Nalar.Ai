...
import { WebTabOpenerContext } from '@/components/chat/home/web-tab-opener'
...
import AddProviderModal from '@/components/chat/home/AddProviderModal'
import type { ProviderConfig } from '@/lib/provider-api'
...
const FilePreviewDrawer = dynamic(() => import('@/components/chat/preview/FilePreviewDrawer'), {
  ssr: false,
})
const SessionViewerPanel = dynamic(() => import('@/components/chat/home/SessionViewerPanel'), {
  ssr: false,
})
...
const CAPABILITIES: CapabilityDef[] = [
  {
    ...
    icon: MessageSquare,
    allowedTools: ['web_search', 'paper_search', 'code_execution', 'reason'],
    defaultTools: [],
  },
  {
    value: 'deep_solve',
  ...
  },
  {
    value: 'journal',
    label: 'Jurnal',
    description: 'Penelitian, penulisan, dan review artikel ilmiah',
    icon: BookOpenCheck,
    allowedTools: ['web_search', 'paper_search', 'code_execution', 'reason'],
    defaultTools: ['paper_search', 'reason'],
  },
  {
    value: 'deep_research',
    label: 'Research',
...
export default function ChatPage() {
  ...
  // Time-of-day greeting — picked once on the CLIENT after mount. The SSR
  // shell renders a stable placeholder ("What would you like to learn?") and
  // the real greeting swaps in client-side only. Picking it during render
  // (useState initializer / useMemo) would differ between server and client
  // HTML → React hydration mismatch (#418) → React throws away the whole
  // tree and re-renders, which drops every piece of session UI state
  // (persona preset, reasoning mode, …) and looks exactly like a refresh.
  ...
  const displaySessionTitle = persistedSessionTitle || firstUserTitle || t('New chat')
  ...
  // Deliberately NOT router.replace: navigating /home → /home/<id> is a
  // route change that remounts this page, which wipes every local piece of
  // UI state (selected capability, config form, menus, …) in the middle of
  // the first turn — the user sees "a refresh" with their selection gone.
  // history.replaceState updates the address bar only, so a browser reload
  // still lands on the right session while React state survives untouched.
  useEffect(() => {
    if (state.sessionId && !sessionIdParam) {
      const url = `/home/${state.sessionId}`
  ...
  const handleProviderCreated = useCallback(
    async (provider: ProviderConfig) => {
      const payload = await refreshLLMOptions()
      const createdOption = payload?.options.find(
        option => option.profile_id === provider.id && option.model === provider.model_name
      )
      if (createdOption) {
        setLLMSelection({
          profile_id: createdOption.profile_id,
          model_id: createdOption.model_id,
        })
      }
      setAddProviderOpen(false)
    },
    [refreshLLMOptions, setLLMSelection]
  )
  ...
  // How many times Nalar AI may consult the selected agent this turn. Seeded
  ...
  const handleSend = useCallback(
    async (content: string) => {
      ...
      // (how many times Nalar AI may ask it) so the subagent capability uses it.
          ...
          <div className="flex w-full flex-1 min-h-0 flex-col">
            {sessionLoading ? (
              ...
              <div
                ...
                style={hasMessages ? { paddingBottom: '48px' } : undefined}
              >
                <div className="mx-auto w-full max-w-[960px] space-y-9 px-6">
                  <WebTabOpenerContext.Provider
                    value={href => viewerPanelRef.current?.openWebTab(href)}
                  >
                    <ChatMessageList
...
function SubagentTabWatcher({
  ...
  useEffect(() => {
    // Group by turn so all of one turn's consults (Nalar AI may ask the agent
