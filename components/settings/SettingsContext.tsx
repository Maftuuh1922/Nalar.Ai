...
export type UiSettings = {
  theme: 'light' | 'dark' | 'glass' | 'snow' | 'ascii'
  language: 'en' | 'zh'
...
const DIAGNOSTICS_RESULTS_KEY = 'nalar-ai.settings.diagnosticsResults.v1'
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const [theme, setTheme] = useState<UiSettings['theme']>('ascii')
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      const activeTheme = getStoredTheme() ?? payload.ui.theme ?? 'ascii'
      setTheme(activeTheme)
      if (activeTheme !== payload.ui.theme) {
        await persistUiSettingsPatch({ theme: activeTheme })
      }
  ...
  const saveCatalog = useCallback(async () => {
    ...
    try {
      ...
      // A failed save used to fall through here and write `payload.catalog ===
      // undefined` into state while still toasting success, which looked exactly
      // like "the settings vanished after saving".
      if (!response.ok) {
        setToast(`${t('Failed to save')} (HTTP ${response.status})`)
        return
      }
