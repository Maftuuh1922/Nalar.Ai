...
function injectBaseHref(html: string, pageUrl: string): string {
  try {
    ...
    if (/<base\s/i.test(html.slice(0, 500))) return html;
    return html.replace(/<head([^>]*)>/i, (m, attrs) => `<head${attrs}><base href="${base}">`);
  } catch {
    return html;
  }
}
...
function WebTabBody({ url }: { url: string }) {
  ...
  const [textDoc, setTextDoc] = useState<string | null>(null);
  ...
  // Keeps the address bar in sync whenever history position changes.
  useEffect(() => {
  ...
  // When the target site refuses to embed (X-Frame-Options / CSP), fetch a
  // rendered copy of the real page via Jina Reader (r.jina.ai) — first as
  // full HTML (so it still looks like a real browser inside the panel),
  // falling back to a clean markdown version if the HTML fetch fails.
  useEffect(() => {
    if (!blocked) return;
    let cancelled = false;
    const ctrl = new AbortController();
    const timer = window.setTimeout(() => ctrl.abort(), 30000);
    const readUrl = `https://r.jina.ai/${encodeURIComponent(currentUrl)}`;
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (!cancelled) {
            setHtmlDoc(injectBaseHref(html.slice(0, 1_500_000), currentUrl));
          }
          return;
        }
        ...
        if (!res2.ok) throw new Error(`reader status ${res2.status}`);
        const text = await res2.text();
        if (!cancelled) setTextDoc(text.slice(0, 300_000));
      } catch {
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Browser-style toolbar: back / forward / refresh + editable address bar */}
      <div className="flex shrink-0 items-center gap-1.5 border-b border-[var(--border)]/40 bg-[var(--card)] px-2.5 py-2">
        <button
          type="button"
          onClick={goBack}
          disabled={!canBack}
          aria-label={t("Back")}
          className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--muted-foreground)] transition-colors hover:bg-[var(--muted)]/45 hover:text-[var(--foreground)] disabled:opacity-35"
        >
        ...
        <button
          ...
          onClick={reload}
          aria-label={t("Reload")}
          className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--muted-foreground)] transition-colors hover:bg-[var(--muted)]/45 hover:text-[var(--foreground)]"
        >
        ...
        >
          <div className="flex items-center gap-1.5 rounded-md border border-[var(--border)]/50 bg-[var(--background)] px-2 py-1 transition-colors focus-within:border-[var(--primary)]/45">
            <Globe size={12} strokeWidth={1.8} className="shrink-0 text-[var(--muted-foreground)]" />
            <input
              value={address}
              onChange={(e) => setAddress(e.target.value)}
              spellCheck={false}
              className="min-w-0 flex-1 bg-transparent text-[12px] text-[var(--foreground)] outline-none placeholder:text-[var(--muted-foreground)]"
              placeholder="https://…"
            />
          </div>
        </form>
      ...
      >
        ...
        <span>
          {blocked
            ? t(
                "This site refused to load directly — showing a fetched copy of the real page instead.",
              )
            : t(
                "Some sites refuse to load inside preview panels. If it stays blank, a fetched copy appears automatically.",
              )}
        </span>
      </div>
      ...
      <div className="relative flex-1 overflow-hidden bg-[var(--background)]">
        {blocked ? (
          htmlDoc != null ? (
            <iframe
              key={`srcdoc-${currentUrl}-${reloadKey}`}
              srcDoc={htmlDoc}
              title={host}
            ...
            <div className="h-full overflow-y-auto px-4 py-3">
              <div className="mb-2 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
                <FileText size={11} strokeWidth={1.8} />
                {t("Text view — auto-fetched from {{host}}", { host })}
              </div>
              <MarkdownRenderer content={textDoc} variant="prose" />
            </div>
          ) : textFailed ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center">
              <Globe size={30} strokeWidth={1.4} className="text-[var(--muted-foreground)]" />
              <div className="max-w-sm text-[12.5px] leading-snug text-[var(--muted-foreground)]">
                {t(
                  "This site blocks embedding and its copy couldn't be fetched. Open it directly in a new browser tab to read it.",
                )}
              </div>
              <a
                href={currentUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-1 inline-flex items-center gap-1.5 rounded-md bg-[var(--primary)] px-4 py-2 text-[12px] font-semibold text-[var(--primary-foreground)] hover:bg-[var(--primary)]/90"
              >
            ...
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-2 bg-[var(--card)]/70 text-[12px] text-[var(--muted-foreground)] backdrop-blur-sm">
              ...
              />
              <span>{t("Fetching a copy of {{host}}…", { host })}</span>
            </div>
          )
        ) : (
          <>
            <iframe
              key={`direct-${currentUrl}-${reloadKey}`}
              src={currentUrl}
              title={host}
