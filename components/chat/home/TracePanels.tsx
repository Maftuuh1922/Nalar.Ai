...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "Nalar AI Reflecting…"
          ...
          {(role === "retrieve" || kind === "math_render_output") &&
            rawProgressEvents.length > 0 && (
              <div className="space-y-0.5">
                ...
                <div className="max-h-[200px] overflow-y-auto rounded-md border border-[var(--border)] bg-[#232427] px-3 py-2 font-mono text-[10px] leading-[1.55] text-[#D6D3D1] shadow-inner">
...
/**
 * Live "Thought for Xs" readout (Beautiful UI Thinking pattern). Counts up
 * from the trace's first event timestamp while the row is the active
 * streaming step, then freezes once the row settles — matching the
 * documentation's "Thought for 4 seconds" label.
 */
function ThoughtTimer({
  startedAt,
  active,
}: {
  startedAt: number;
  ...
  // Date.now() only inside effects — a time-based useState initializer
  // differs between SSR and client render → hydration mismatch (#418).
...
function TraceRowItem({
  ...
  const startedAt = Number(first?.timestamp) || 0;
  ...
  // Thinking rows pin open while live; once settled the user can fold them
  // back to the "Thought for Xs" line (Beautiful UI Thinking pattern).
  const open = isThinking ? (userOpen ?? true) : expandable && (userOpen ?? autoOpen);
  const canToggle = expandable && (!isThinking || !active);
    ...
    <div className="group/row">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          {showChatBody ? (
            <div className="relative ml-[5px] space-y-1.5 border-l border-[var(--border)]/50 pl-3 italic leading-[1.6]">
              {[thoughtText, contentText]
                .filter(Boolean)
                .map((text, idx) => (
                  <div
                    key={idx}
                    className="bu-fade-up"
                    style={{ animationDelay: `${idx * 80}ms` }}
                  >
                    <MarkdownRenderer content={text} variant="trace" />
                  </div>
                ))}
            </div>
          ) : isThinking ? (
            // Pipeline "Thought"/"Plan" rounds: a live "Thought for Xs"
            // readout while streaming (Beautiful UI pattern), then the
            // model's reasoning below it — never folded while active. The
            ...
            <>
              ...
              >
                {active ? (
                  <>
                    {t("Thought for")}{" "}
                    <ThoughtTimer startedAt={startedAt} active={active} />
                  </>
                ) : (
                  headline
                )}
              ...
              {thoughtText || contentText ? (
                <div className="relative mt-1 border-l border-[var(--border)]/50 pl-3 leading-[1.6]">
                  <div className="bu-fade-up">
                    <MarkdownRenderer
                      ...
                      <div key={trace.callId} className="mt-1.5 pl-0.5">
                        ...
                        <div className="mt-0.5 space-y-0.5">
                          ...
                          {(trRole === "retrieve" ||
                            ...
                            rawEvts.length > 0 && (
                              <div className="max-h-[160px] overflow-y-auto rounded-md border border-[var(--border)] bg-[#232427] px-3 py-2 font-mono text-[10px] not-italic leading-[1.55] text-[#D6D3D1] shadow-inner">
                                {rawEvts.map((ev, ei) => (
                                  <div
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "Nalar AI Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  // Time-based initializer would mismatch SSR ↔ client → hydration error.
  ...
  const name = agentName?.trim() || "Nalar AI";
  ...
  else if (mode === "responded") modeLabel = t("Nalar AI responded.");
 ...
 * reasoning trace once Nalar AI stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("Nalar AI Exploring… · 8s", settling to "Nalar AI responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while Nalar AI is still reasoning /
