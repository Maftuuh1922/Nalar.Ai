...
export default memo(function ChatComposer({
  ...
  /** Max times Nalar AI may consult the selected agent this turn. */
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      if (composerRef.current) {
        setComposerCompact(composerRef.current.getBoundingClientRect().width < 620)
      }
    })
    observer.observe(el)
  ...
  const canSend = (hasContent || hasReferences) && !isStreaming && !isConfigBlocked
  ...
  const handleJournalPreset = useCallback(
    (preset: JournalWorkflowPreset) => {
      onSelectCapability('journal')
      inputHandleRef.current?.setValue(preset.prompt)
      setHasContent(true)
          ...
          <div className="px-3 pb-2 pt-0.5">
            <div className="flex flex-wrap items-center gap-1 sm:flex-nowrap">
                ...
                {capMenuOpen && (
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      const loopSelected = loopCaps.some(cap => cap.value === activeCap.value)
                      return (
                        <div
                        ...
                        >
                          <button
                            ...
                            } ${loopSelected && !moreCapsOpen ? 'bg-[var(--primary)]/[0.06]' : ''}`}
                          >
                            <Sparkles
                              size={15}
                              strokeWidth={1.7}
              ...
              <div className="relative flex min-w-0 flex-1 items-center">
                ...
                <JournalWorkflowMenu onSelect={handleJournalPreset} compact={false} />
              </div>

              <div className="ml-auto flex shrink-0 items-center gap-1.5 max-sm:order-3 max-sm:mt-1 max-sm:w-full max-sm:justify-end max-sm:border-t max-sm:border-[var(--border)]/30 max-sm:pt-1">
                ...
                {isStreaming ? (
                  ...
                  <button
                    ...
                    title={
                      isConfigBlocked ? t('Confirm settings on the right to send.') : undefined
                    }
