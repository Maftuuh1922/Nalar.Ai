...
import SaveReferenceFromMessage from "./SaveReferenceFromMessage";
import ExportToCoWriterButton from "./ExportToCoWriterButton";
...
/** "Nalar AI_Introduction.pdf" → "Nalar AI Introduction" — the card title
  ...
  return (
    <>
      {/* Activity block pinned to the TOP: the status header
          ("Nalar AI Exploring… · 8s" → "Nalar AI responded. · 10s") with
          the exploring trace nested beneath it — expanded while Nalar AI is
      ...
      {/* Simpan link jurnal dari pesan AI ke Referensi (grup aktif). */}
      {!isStreaming && msg.content ? (
        <SaveReferenceFromMessage content={msg.content} />
      ) : null}
      {/* Ekspor sesi chat ini ke Co-Writer (draf baru). */}
      {!isStreaming && sessionId && msg.content ? (
        <ExportToCoWriterButton sessionId={sessionId} />
      ) : null}
    </>
  );
});
        ...
        return (
          <div key={`${msg.role}-${i}`} className="w-full">
            ...
            {(() => {
              ...
              // Some backend paths (e.g. provider connection failure) emit
              // a plain `error` event without the terminal flag, so fall
              // back to any error event on a finished message.
              ...
              const diedError =
                terminalError ||
                events.find(
                  (e) => e.type === "error" && Boolean(e.content),
                );
              if (!diedError) return null;
              return (
                <div className="mt-3 flex w-full max-w-[min(520px,90%)] items-center gap-2 rounded-xl border border-[var(--destructive)]/30 bg-[var(--destructive)]/5 px-3 py-2">
                  ...
                  <span className="min-w-0 flex-1 text-[12px] leading-[1.5] text-[var(--foreground)]">
                    {diedError.content || t("The turn was interrupted.")}
                  </span>
                  {showRegenerate ? (
                    <button
                      type="button"
