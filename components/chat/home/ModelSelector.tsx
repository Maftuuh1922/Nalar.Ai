...
import { AlertCircle, Bot, Check, ChevronDown, Plus } from "lucide-react";
...
export default function ModelSelector({
  ...
  const disabled =
    loading || (options.length === 0 && !allowSystemDefault && !error);
  const label = loading
    ? t("Loading models")
    : error
    ...
    <div ref={rootRef} className="relative">
      ...
      {open && !loading && (
        ...
        >
          ...
          {onAddProvider ? (
            <div className="border-b border-[var(--border)]/50 p-1">
              <button
                type="button"
                onClick={() => {
                  onAddProvider();
                ...
                className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[12.5px] font-medium text-[var(--primary)] transition-colors hover:bg-[var(--primary)]/[0.07]"
              >
