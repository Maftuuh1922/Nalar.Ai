...
// exports `NALAR_AI_API_BASE_URL` from `data/user/settings/system.json`
// defaults to `http://localhost:8001`. In dev it
// defaults to `http://localhost:8001`.
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_BASE ??
  process.env.NALAR_AI_API_BASE_URL ??
  "http://localhost:8086";

const AUTH_ENABLED = parseAuthEnabled(process.env.NALAR_AI_AUTH_ENABLED);
