...
export async function listNotebooks(): Promise<NotebookSummary[]> {
  const response = await apiFetch(apiUrl("/api/v1/notebooks"), {
    cache: "no-store",
  });
...
export async function getNotebook(notebookId: string): Promise<NotebookDetail> {
  const response = await apiFetch(apiUrl(`/api/v1/notebooks/${notebookId}`), {
    cache: "no-store",
  });
  if (!response.ok) throw new Error(`Request failed: ${response.status}`);
...
export async function listNotebookEntries(
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/questions/entries${query ? `?${query}` : ""}`),
    { cache: "no-store" },
  );
...
export async function getNotebookEntry(
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/questions/entries/${entryId}`),
    {
      cache: "no-store",
    },
  );
...
export async function lookupNotebookEntry(
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/questions/entries/lookup/by-question?${params}`),
  );
...
export async function upsertNotebookEntry(data: {
  ...
  const response = await apiFetch(
    apiUrl("/api/v1/questions/entries/upsert"),
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
export async function addEntryToCategory(
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/questions/entries/${entryId}/categories`),
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function removeEntryFromCategory(
  ...
  const response = await apiFetch(
    apiUrl(
      `/api/v1/questions/entries/${entryId}/categories/${categoryId}`,
    ),
    { method: "DELETE" },
  );
...
export async function listCategories(): Promise<NotebookCategory[]> {
  const response = await apiFetch(
    apiUrl("/api/v1/questions/categories"),
    {
      cache: "no-store",
    },
  );
...
export async function renameCategory(
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/questions/categories/${categoryId}`),
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name }),
