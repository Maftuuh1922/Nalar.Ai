...
export async function listSkills(options?: {
  ...
  return withClientCache<SkillInfo[]>(
    ...
    async () => {
      const response = await apiFetch(apiUrl("/api/v1/skills"), {
        cache: "no-store",
      });
...
export async function getSkill(name: string): Promise<SkillDetail> {
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/skills/${encodeURIComponent(skill.id)}`),
    {
      cache: "no-store",
    },
  );
...
// skills in Nalar AI's own UI and download them with one click.
