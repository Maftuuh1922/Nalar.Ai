...
export function toggleTheme(currentTheme: Theme): Theme {
  const order: Theme[] = ['ascii', 'snow', 'light', 'dark', 'glass']
...
export function getThemeClass(theme: Theme): string {
  ...
  if (theme === 'ascii') return 'theme-ascii'
  return ''
}
...
export function getBackgroundForTheme(theme: Theme): string {
  return theme === 'dark' || theme === 'glass' ? 'dark:bg-slate-800' : 'bg-white'
}
