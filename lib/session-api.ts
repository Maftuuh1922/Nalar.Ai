  ...
  return withClientCache<SessionSummary[]>(
    ...
    async () => {
      const response = await apiFetch(
        apiUrl(`/api/v1/chat/sessions?${qs.toString()}`),
        {
          cache: "no-store",
        },
      );
      const data = await expectJson<SessionSummary[]>(response);
      return data ?? [];
    },
    {
      force: options?.force,
...
export async function getSession(
  ...
  const response = await apiFetch(apiUrl(`/api/v1/chat/sessions/${sessionId}`), {
    cache: "no-store",
    signal,
  });
...
export async function updateSessionTitle(
  ...
  const data = await expectJson<SessionDetail>(response);
...
export async function deleteSession(sessionId: string): Promise<void> {
  ...
  await expectJson<{ deleted?: boolean; status?: string }>(response);
  invalidateClientCache("sessions:");
}
...
export async function recordQuizResults(
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/chat/sessions/${sessionId}/quiz-results`),
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function deleteMessage(
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/chat/sessions/${sessionId}/messages/${messageId}`),
    { method: "DELETE" },
  );
...
export async function updateBranchSelection(
  ...
  const response = await apiFetch(
    apiUrl(`/api/v1/chat/sessions/${sessionId}/branch-selection`),
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
